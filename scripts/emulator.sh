...
if [ ! -f /system/build.prop ]; then
  # Running on PC
  cd "$(dirname "$0")/.."
  adb push native/out/x86_64/busybox native/out/x86_64/joiskinit \
...
if $SELINUX; then
  ln -sf ./joiskinit joiskpolicy
  ./joiskpolicy --live --magisk
fi
...
ln -s ./magisk $BINDIR/jojo
ln -s ./magisk $BINDIR/josetprop
