...
import com.brightsight.joker.ktx.reboot
    ...
    private fun extractFiles(): Boolean {
        ...
        try {
            ...
            if (isRunningAsStub) {
                ...
                // Also extract joisk32 on non 64-bit only 64-bit devices
                val is32lib = Const.CPU_ABI_32?.let {
                    { entry: ZipEntry -> entry.name == "lib/$it/libjoisk32.so" }
                } ?: { false }
        ...
        override suspend fun exec(): Boolean {
            ...
            if (success) {
                UiThreadHandler.handler.postDelayed(3000) {
                    Shell.jojo("pm uninstall ${context.packageName}").exec()
                }
            }
            return success
        }
