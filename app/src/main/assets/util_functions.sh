...
getvar() {
  ...
  [ ! -z $JOKERTMP ] && PROPPATH="$JOKERTMP/config $PROPPATH"
...
remove_system_su() {
  if [ -f /system/bin/su -o -f /system/xbin/su ] && [ ! -f /su/bin/su ]; then
    ...
    /system/xbin/daemonsu /system/xbin/su /system/xbin/sugote /system/xbin/sugote-mksh /system/xbin/jojopolicy \
...
find_magisk_apk() {
  ...
  [ -f $APK ] || APK=/data/app/com.brightsight.joker*/*.apk
  [ -f $APK ] || APK=/data/app/*/com.brightsight.joker*/*.apk
  if [ ! -f $APK ]; then
...
run_migrations() {
  ...
  for name in boot dtb dtbo dtbs; do
    ...
    [ -f $BACKUP ] || continue
    if [ $name = 'boot' ]; then
      LOCSHA1=`$JOKERBIN/magiskboot sha1 $BACKUP`
      mkdir /data/magisk_backup_${LOCSHA1} 2>/dev/null
    fi
