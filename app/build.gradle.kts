...
android {
    defaultConfig {
        applicationId = "com.brightsight.joker"
...
val syncLibs by tasks.registering(Sync::class) {
    ...
    into("armeabi-v7a") {
        from(rootProject.file("native/out/armeabi-v7a")) {
            include("busybox", "magiskboot", "joiskinit", "magisk")
            rename { if (it == "magisk") "libjoisk32.so" else "lib$it.so" }
        }
    }
    into("x86") {
    ...
    into("arm64-v8a") {
        from(rootProject.file("native/out/arm64-v8a")) {
            include("busybox", "magiskboot", "joiskinit", "magisk")
            rename { if (it == "magisk") "libjoisk64.so" else "lib$it.so" }
        }
    }
    into("x86_64") {
...
val syncAssets by tasks.registering(Sync::class) {
    ...
    filesMatching("**/util_functions.sh") {
        filter {
            it.replace("#JOKER_VERSION_STUB",
                "JOKER_VER='${Config.version}'\n" +
                "JOKER_VER_CODE=${Config.versionCode}"
            )
        }
...
dependencies {
    ...
    val vLibsu = "3.1.3"
    implementation("com.github.Joyoe.libsu-jojo:core:${vLibsu}")
    implementation("com.github.Joyoe.libsu-jojo:io:${vLibsu}")
