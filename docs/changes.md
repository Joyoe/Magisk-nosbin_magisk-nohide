...
- [JoiskInit] Fix `sepolicy.rule` mounting strategy
...
- [JoiskInit] Support Galaxy S21 series
...
- [JoiskInit] Avoid mounting `f2fs` userdata as it may result in kernel crashes. This shall fix a lot of bootloops
...
- [JoiskInit] Support kernel cmdline `androidboot.fstab_suffix`
- [JoiskInit] Support kernel initialized dm-verity on legacy SAR
...
- [JoiskInit] Rewritten 2SI support from scratch
- [JoiskInit] Support when no `/sbin` folder exists (Android 11)
- [JoiskInit] Dump fstab from device-tree to rootfs and force `init` to use it for 2SI devices
- [JoiskInit] Strip out AVB for 2SI as it may cause bootloop
...
- [JoiskInit] Support recovery images without /sbin/recovery binary. This will fix some A/B devices unable to boot to recovery after flashing Magisk
...
- [JoiskInit] [2SI] Support non A/B setup (Android 10)
- [MagiskHide] Fix bug that reject process names with ":"
- [MagicMount] Fix a bug that cause /product mirror not created

### v19.4

- [JoiskInit] [SAR] Boot system-as-root devices with system mounted as /
- [JoiskInit] [2SI] Support 2-stage-init for A/B devices (Pixel 3 Android 10)
- [JoiskInit] [initramfs] Delay sbin overlay creation to post-fs-data
- [JoiskInit] [SARCompat] Old system-as-root implementation is deprecated, no more future changes
- [JoiskInit] Add overlay.d support for root directory overlay for new system-as-root implementation
...
- [JoiskInit] Wait for partitions to be ready for early mount, should fix bootloops on a handful of devices
- [JoiskInit] Support EROFS used in EMUI 9.1
...
- [JoiskInit] Add Kirin hi6250 support
...
- [JoiskInit] New sbin overlay setup process for better compatibility
- [JoiskInit] Allow long pressing volume up to boot to recovery in recovery mode
...
- [JoiskInit] Support Android Q (no logical partition support yet!)
- [MagiskPolicy] Support Android Q new split sepolicy setup
- [JoiskInit] Move sbin overlay creation from main daemon post-fs-data to early-init
- [General] Service scripts now run in parallel
- [JoiskInit] Directly inject magisk services to init.rc
...
- [JoiskInit] Randomize Magisk service names to prevent detection (e.g. FGO)
...
- [JoiskInit] Move all sepolicy patches pre-init to prevent Pixel 2 (XL) boot service breakdown
...
- [JoiskInit] Detect early mount partname from fstab in device tree
...
- [Daemon] Check whether a valid Magisk Manager is installed on boot, if not, install stub APK embedded in joiskinit
- [Daemon] Check whether Magisk Manager is repackaged (hidden), and prevent malware from hijacking com.brightsight.joker
...
- [JoiskInit] Support detecting block names in upper case (Samsung)
...
- [JoiskInit] Remove `joiskinit_daemon`, the actual magisk daemon (magiskd) shall handle everything itself
...
- [JoiskInit] Support non `skip_initramfs` devices with slot suffix (Huawei Treble)
...
- [JoiskInit] Prevent OnePlus angela `sepolicy_debug` from loading
- [JoiskInit] Obfuscate Magisk socket entry to prevent detection and security
...
- [JoiskInit] Support non `skip_initramfs` devices using split sepolicies (e.g. Zenfone 4 Oreo)
...
- [Daemon] New invincible implementation: use `joiskinit_daemon` to monitor sockets
...
- [JoiskInit][MagiskBoot] Support ramdisk high compression mode
...
- [Magiskinit] Use joiskinit for all devices (dynamic initramfs)
...
- [JoiskInit] Introduce a new init binary to support `skip_initramfs` devices (Pixel family)
