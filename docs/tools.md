...
Note: The Magisk zip you download only contains `magiskboot`, `joiskinit`, and `joiskinit64`. The binary `magisk` is compressed and embedded into `joiskinit(64)`. Push `joiskinit(64)` to your device and run `./joiskinit(64) -x magisk <path>` to extract `magisk` out of the binary.
